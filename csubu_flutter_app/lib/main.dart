...
class _AppHomePageState extends State<AppHomePage> {
  ...
  _getStudents() async {
    var url = 'http://cs.sci.ubu.ac.th:7512/topic-1/59110440345/_search?from=${_page*10}&size=10';
    ...
    if (response.statusCode == 200) {
      ...
      result.forEach((item) {
        if (item.containsKey('_source')) {
          ...
          if (source.containsKey('name') && source.containsKey('no')) {
  ...
  Widget studentWidgets(BuildContext context) {
    return ListView.separated(
        ...
        itemBuilder: (context, i) {
          ...
          var sum = 0;
          student['no'].runes.forEach((c) { sum += c; });
          return ListTile(
            title: Row(
                  children: <Widget>[
